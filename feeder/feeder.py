...
class Feeder(torch.utils.data.Dataset):
    ...
    def reshape(self):
        ...
        new_shape = (init_shape[0], init_shape[2], init_shape[4] * init_shape[3] * init_shape[1])
        data = np.zeros(shape = new_shape)
        for clip in tqdm(range(init_shape[0])):
            for t in range(init_shape[2]):
                for body in range(init_shape[4]):
                    for joint in range(init_shape[3]):
                        for axes in range(init_shape[1]):
                            bias = body * init_shape[3] * init_shape[1] + joint * init_shape[1] + axes
                            data[clip, t,  bias] = self.data[clip, axes, t, joint, body]
        self.data = data
    ...
    def load_data(self):
        ...
        if 'PKUMMD' in self.data_path:
            if self.downsampling:
                self.data = self.data[:,::5,:]
                self.valid_frame_num = self.valid_frame_num // 5
        if self.debug:
    ...
    def coordinate_transfer(self):
        ...
        if self.origin_transfer == 2:
            ...
            origin = data_numpy[:, :, :, 1, 0] # (batch, 3, 300, 25, 2)
            data_numpy = data_numpy - origin[:, :, :, None, None] # do it on sequence level
            print("==> origin transfer")
...
if __name__ == '__main__':
    ...
    data_path = "/home2/wuxiao/va_lstm/va_lstm/dataset/NTU-RGB-D/xview/val_data.npy"
    label_path = "/home2/wuxiao/va_lstm/va_lstm/dataset/NTU-RGB-D/xview/val_label.pkl"
